function [bark_index,bark_mag,bark_count] = calculate_nm(is_nm,audio_segment,bark_lims,freq_res)
    ...
    for i=1:length(audio_segment)
        if(is_nm(1,i)==1)
            cur_bark=getbark(i*freq_res,bark_lims);
    ...
    for i=1:length(bark_lims)
        bark_index(1,i)=round(nthroot(bark_index(1,i),bark_count(1,i)));
    end
